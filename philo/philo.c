...
int	start_dining(t_maitred *maitre_d)
{
	...
	i = 0;
	if (pthread_create(&maitre_d->maitre_d, NULL, &oversee,
			(t_maitred *)maitre_d) != 0)
	{
	...
	{
		if (pthread_create(&maitre_d->philo[i]->philo, NULL, &routines,
				(t_philo *)maitre_d->philo[i]) != 0)
		{
...
int	seat_philosophers(t_maitred *maitre_d)
{
	...
	maitre_d->philo[0]->right_fork
		= &maitre_d->philo[maitre_d->number_of_philos - 1]->left_fork;
	return (0);
}
